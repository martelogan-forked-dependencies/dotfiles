...
function macloc --description "fzf with locatef"
  if not type -q ggrep
    echo "requires ggrep"
    return
  end
...
function clonestall --description "clone something, cd into it. install it."
    if not type -q thefuck
      echo "requires thefuck"
      return
    end
...
function mkd --wraps mkdir -d "Create a directory and cd into it"
...
function gzsz --d "Get the gzipped size"
...
function server -d 'Start a HTTP server in the current dir, optionally specifying the port'
    if not type -q statikk
      echo "requires statikk"
      return
    end
    if test $argv[1]
        set port $argv[1]
